...
- 자기의 조장이 누구인지 나 자신을 조회해서 조장의 ID를 조회?
...
- 문제 1 : 
	...
	- N.TITLE, N.WRITER_ID, M.NAME, M.NICNAME, COUNT(C.ID) CMT_COUNT에서 NOTICE의 위치에서 MEMBER를 조회하면 오른쪽에 OUTER가 붙는다!
	- SELECT N.TITLE, N.WRITER_ID, M.NAME, M.NICNAME, COUNT(C.ID) CMT_COUNT에서 COMMENT의 위치에서 MEMBER를 조회하면 왼쪽에 OUTER가 붙는다!
	- 보통 주인공을 조회할 때, 앞에서 조회하기 때문이다!!!
...
- Cross join : 멤버 테이블과 코멘트 테이블의 곱이다.
...
- 보통 UNION을 쓸 때는 TYPE을 써주기 때문에 겹치는 경우가 적다.
...
- 3. MINUS : 공통된 부분을 빼고 나머지 부분을 조회한다.
...
- VIEW는 결합 쿼리문만 있어야한다. ORDER BY나 GROUP BY가 없어야 한다! 
- 1개의 테이블을 사용할 때, 1개의 테이블에서 가리고 싶은 경우, VIEW로 따로 만들어 준다! 
- 사용자 별로 허락된 테이블을 보여주고 싶을 때!!
...
- sql에서는 마스터 DB라고 한다. mysql에서는 시스템 DB라고 한다. 오라클에서는 데이터 딕셔너리라고 한다.
...
<br><br>
### 9) 서블릿 레이어 나누기 
- view -  service - dao - dto 
- 하지만 사용자 요청하는 곳에서는 db를 쿼리하지 않느다.
- 사용자 입출력과 업무 로직과 DB쿼리가
- 협업이 안 된다. 그래서 협업을 위해서 나눠서 코드를 작성한다!
