...
각 단계의 페이지 테이블이 메모리에 존재하므로 logical address의 physical address 변환에 더 많은 메모리 접근 필요(4단계 테이블을 사용하면 4번 접근해야 하므로 4배의 시간이 걸림)
	- 4번 접근하고 1번은 주소 변경을 하기 때문에 총 5배의 시간이 걸린다. 아래의 계산에서 이용한다.
...
<br> 
- 메모리에 접근하는 주소변환(물리,논리적 주소)은 cpu가 해서 하드웨어가 해준다.
- 운영체제는 메모리 접근이 아니라 Io장치에 접근할때 이용한다.
- 사용자 프로세스가 직접 IO 장치에 접근하지 못해서 운영체제가 대신 접근해준다.

<br>
- 그래서, 디스크 접근은 cpu가 운영체제로 넘어가고
- 메모리 접근에서는 매번 cpu가 운영체제로 넘어가는 것은 번잡해져서 말이 안된다.

<br>
- 최종 결론 : 이번 메모리 접근 챕터에서는 운영체제의 역할은 없었다.**
	- 다음 챕터인 Virtual Memory에서는 운영체제가 관여한다.
...
#### a. Physical formatting(low-levlel formatting)
