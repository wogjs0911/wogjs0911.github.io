...
- MAC adress : 웹에서는 lan port address(랜카드)에 의해 식변된다.(media access control address = MAC adress) : web address
	- 192.168.0.1에서 1은 host 번호이고 1에 의해서 MAC address가 정해진다.
	
- broad casting : 그래서 방송을 한다. network 환경 설정에서 32번으로 설정되어 있으면  이것이 host 번호이다.

- port : 아직은 밖에 있는 데이터가 컴퓨터까지만 와서 추가적으로 데이터가 있는데 이것이 port라고 부르며 이것은 사서함과 같다. 
	- http에서 8080이라는 포트번호를 사용에서 포트번호는 각 역할을 부여해준다.
	
- 정리** : 데이터가 올 때는, 웹에서 ip로 오지만 port번호가 있어서 사서함에 데이터를 전해준다. ip는 문턱까지, mac은 디테일까지, port는 각 역할까지
...
<br>
### 7) [WAS]
	- 문서화 하기 위해 코드를 모아두는 공간을 "server app"이라고 하며 서버를 실행해주는 실행환경 그자체는 "Web server"라고 하며 그것에 대한 코드를 실행하여 그 결과를 돌려주는 서버를 "WAS"라고 한다. WAS는 서버라는 타입명이다. 그중에서 제품명은 톰캣이 있다. 	
...
- tomcat을 실행하기 위해서는 java를 이용해서 JAVA_HOME이라는 환경 변수를 설정해야 한다.

- startup.bat 파일을 키면, 브라우저와 클라이언트를 연결할 수 있다. 즉 서버와 같은 역할.

- D:\Setup\apache-tomcat-10.0.27\webapps\ROOT의 파일이 브라우저에서 표시된다.

- ROOT 폴더에서 nana.txt를 만들면, http://localhost:8080/nana.txt로 브라우저로 이동하면 txt 파일을 브라우저에서 확인할 수 있다.
