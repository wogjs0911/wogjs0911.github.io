...
<br>
- 정규 표현식을 DB에서 사용하는 방법, 이것이 더 안전한 이유 :
	- `REGEXP_LIKE`
	- `REGEXP_REPLACE`	
	- 1) sql문의 where 절에서 정규표현식의 조건 판단에 사용할 수 있다 : `SELECT * FROM MEMBER WHERe NAME LIKE '%모%';`
	- 2) sql문의 where 절에서 정규표현식의 조건 판단에 사용할 수 있다 : `SELECT * FROM MEMBER WHERE REGEXP_LIKE(NAME, '^.*모.*&');	`
...
<br>	
- COUNT 집계 함수 :  null이 들어가지 않는 데이터칼럼을 찾아서 집계해야 한다.
	- COUNT(*)보다는 COUNT(ID)가 좋다.
...
```
SELECT 
    (SELECT NICNAME FROM MEMBER WHERE ID=WRITER_ID) WRITER_NAME,
    COUNT(ID) COUNT
FROM NOTICE
...
<br>
- HAVING : 집계함수 이후에 조건을 정해주기 위해서 사용된다.
