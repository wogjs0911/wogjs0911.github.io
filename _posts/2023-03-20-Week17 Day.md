...
- 테이블의 칼럼명 수정 : 위의 내용대로 설계하면 rcmd_menu_id인데 이것은 추천 메뉴 테이블의 id를 의미하므로 그냥 id라고 사용해도 된다!! 그래서, SQL문 조건은 식별되어야 하기 때문에 `rm.id`로 비교해야 한다. rm.menu_id가 아니라 바뀐 칼럼명인 `rm.id`로 비교해야 한다.
