...
#### a) 다대다 관계의 DB 모델링의 key의 포함 관계 정리

- 시험 문제를 내는 사람이 같은 사람이 문제를 등록하면 등록하는 것에 포함시키지 않는다.

- 예약을 하고 결제를 하는 사람이 같은 사람으로서 예약하는 사람이라면 결제를 하면 예약자에 결제를 FK로 포함시키지 않는다.

- 사전에 하는일이 4형식 문형인데 같은 일을 하는게 같은 사람이라면 그 사람은 FK에 포함시키지 않는다.
	
- 결론 : 같은 키를 2개로 FK로 받아서 이름만 바꿔서 사용한다?** 여기서는 즉, 메뉴가 메뉴를 가리킨다. 마치 게시글의 댓글과 같다? 

- 아니다. 다대다 관계라서 게시글과 댓글의 관계가 아니다. 메뉴가 자신인 메뉴를 추천해서 자기가 자신을 가리키는 다대다 관계이다.

- 그래서 : 추천과 구성을 의미하는 rcmd_menu_id로서 id를 정한다. 원래 키는 menu_id이다.
	
- 의문점 : 대리키가 식별키로서 사용되야 하는가? 내 생각은 안 그래도 될 거 같다... 
	
- 식별이 가능하면 대리키인 id가 필요가 없다. 따라서, 대리키인 id가 필요가 없다. menu_id와 rcmd_menu_id(recommend 메뉴에 포함되는 id)만 필요하다! 이 2개가 PK가 된다. 여기에 추천의 궁합도나 설명이라는 칼럼을 추가해줄 수 있다.
