...
- (참고 페이지 1)[https://mvnrepository.com/artifact/org.apache.tiles/tiles-jsp/3.0.8](https://mvnrepository.com/artifact/org.apache.tiles/tiles-jsp/3.0.8)) 
	- 최근버전으로 디펜던시에 추가해주자.
...
- [https://tiles.apache.org/framework/tutorial/basic/pages.html](https://tiles.apache.org/framework/tutorial/basic/pages.html)
...
### 1) 타임 리프 태그 : replace vs insert

- replace : replace는 div 태그로 감싸고 있는데 감싸고 있는 div 태그를 완전 대체 해버린다.

- fragment : "::"에 의해서 따로 떼서 붙여주고 replace로 완전 대체 해준다. 
	- html에서는 이러한 fragment를 완전 명명한다.
- 타임리프는 속성으로 쓰기 때문에 속성을 쓰기 위해서 div 태그에 사용한다
...
- decorate : 알맹이를 꾸며주는 것을 "layout"이라고 한다. 
	- 그래서, list.html에서 바깥을 꾸며주는 것인 header, footer가 있는 부분인 layout.html로 이동한다.


---

<br>
- 실습 코드(list.html 만들어주기) :

- MenuController.java 경로 수정
...
### 3) SpringBoot에서 mariaDB의 VIEW 테이블 이용하기 
