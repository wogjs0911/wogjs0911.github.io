...
- 현재, 이 노드 API가 연결할 때, 데이터베이스와 통신하기 때문에, 데이터베이스를 포함하는 컨테이너를 가동하여 이것이 작동하려면, 이 포트 27017를 노출해야 합니다.   

<br>
- 도커 컨테이너에서 로컬 머신으로 가동시켜야 합니다. 우리 서비스가 로컬 머신을 통해 서비스에 연결할 수 있습니다. 
...
<br>
- 이 컨테이너는 동일한 네트워크의 일부이며, 다른 컨테이너와 통신할 수 있습니다. 
...
<br>
- 첫 번째 노드 앱와 통신해야 하는 두 번째 노드 애플리케이션이 있다면, 컨테이너 이름을 사용할 수 있습니다.

<br>
- 하지만 이것은 브라우저에서 실행되는 JavaScript 코드이므로 도커가 아니라, 브라우저가 이해할 수 있는 코드가 필요합니다.
...
- 컨테이너에서 실행되는 부분인 개발 서버는 네트워크를 신경 쓰지 않기 때문입니다. 

<br>
- 그 개발 서버는 노드 API 또는 데이터베이스와 상호 작용하지 않습니다. 그리고 API와 상호작용하는 부분은 도커 환경에서 실행되지 않습니다.
...
<br>
- 이번에는 환경 변수도 추가하여 실행한다는 거죠.
	- 그리고 첫 번째로 추가하고자 하는 것은	`MONGO_INITDB_ROOT_USERNAME`인데 'max'로 설정하도록 하죠
	- 그리고 두 번째 환경 변수를 한다. `MONGO_INITDB_ROOT_PASSWORD`인데 이건 'secret'라고 할당하죠.

<br>
- 에러 1 : MongoDB 환경변수 설정
	- MongoDB가 이해하는 특수한 포맷을 갖기 때문입니다.
	- 그 포맷은 사용자 이름과 비밀번호를 가지며
	- 호스트 주소 앞에 '사용자 이름:비밀번호@' 형식으로 추가합니다.
	- 비밀번호 다음엔 '@'기호를 추가하는군요. (@, at이므로 호스트 주소의 사용자계정이란 의미)
	- 이 계정설정은 옵션이지만 지금 우리에겐 필요합니다.
	- 사용자 이름과 비밀번호가 있으니까요.
	- 그래서 이 경우에는 'max:secret@mongodb'가 됩니다.

<br>
- 에러 2 : 또 다른 백엔드 에러 발생. 공식 MongoDB 문서에서 그 해결책을 찾을 수 있습니다. 
	- 연결 문자열 끝에 이 'authSource=admin' 쿼리 매개변수를 추가해야 합니다.
...
<br>
- 이미지를 리빌드하면 이러한 중복 파일이 복사되지 않고 이미지가 빌드될 거라 보장할 수 있죠.
...
<br>
- 그리고 'npm install'에 의해 설치된 최신 종속성이 아니라 오래된 node_modules 폴더를 복사할 위험도 있습니다.
...
- 도커 컴포즈는 'docker build'와 'docker run' 명령을 대체할 수 있는 도구입니다.

<br>
- 다수의 'docker build' 명령과 다수의 'docker run'을 단, 하나의 구성 파일로 가집니다.

<br>
- 그것은 모든 서비스 모든 컨테이너를 즉시 시작하고 필요하다면 모든 필요한 이미지를 빌드하는 오케스트레이션 명령 셋(set)입니다. (Orchestration, 자동화된 설정, 구글링 해보기!)

<br>
- 하나의 명령을 사용하여 모든 것을 중지하고 모든 것을 중단할 수도 있습니다. 우리가 선택한 텍스트 파일에 도커 명령을 저장할 수 있기 때문에 이것은 굉장하고 중요하다.

<br>
- 구성 파일을 활용한 단 하나의 명령으로 전체 다중 컨테이너 애플리케이션을 시작하거나 중단할 수 있습니다.

<br>
- 참고적으로 단일 컨테이너 애플리케이션에서도 도커 컴포즈를 사용할 수 있습니다. 하지만, 컨테이너가 여러 개인 경우에 가장 유용하죠.
...
<br>
- 단, 하나의 컴퓨터인 호스팅 머신에서 모든 것을 실행하고 있습니다. 현재 하나의 호스트이므로 도커 컴포즈를 사용하는데 딱이죠.
...
- 가장 중요한 요소 항목은 이른바 서비스(Service)라 불리우는 겁니다. 이는 결국 다중 컨테이너 애플리케이션을 구성하는 컨테이너라고 할 수 있죠.

<br>
- 모든 서비스 아래에서 해당 서비스를 구성할 수 있습니다. 여기서 서비스라고 하면 실제로 컨테이너를 의미하는 겁니다.

<br>
- Docker Compose로 할 수 있는 것들** :
	- 게시해야 하는 포트를 정의하고 이 컨테이너에 필요한 환경 변수를 정의할 수 있습니다.
	- 이 컨테이너에 할당해야 하는 볼륨을 정의할 수 있구요.
	- 네트워크를 할당할 수 있습니다.

<br>
- 정리하면, 기본적으로 터미널에서 도커 명령으로 할 수 있는 모든 것을 도커 컴포즈에서 할 수 있습니다.

<br>
- 터미널에서 이러한 도커 명령을 실행하는 것을 대체하는 것이 컴포즈의 아이디어이기 때문이죠.
...
- 'build: ' 설정 : 
	...
	- 그러면 더 높은 수준의 폴더로 설정되어야 합니다.
	- 그리고 그런 경우는 매우 헷갈립니다. 나중에 여러 
	
<br>
- 'args: ' 설정 : 
	- 경로가 더 긴 형태에서는 여기에 args를 추가하여 그 아래에서 some-arg에 특정 값을 추가할 수 있습니다.
	- Dockerfile이 ARGS를 사용하는 경우에 말이죠. 도커 컴포즈를 사용할 때 이렇게 args를 지정할 수 있는데요.
...
- 'docker-compose up'명령에 '--build'를 추가하여 이를 강제할 수 있습니다.
...
- 커스텀된 이미지로 생성 : `docker-compose build`
	- 자체 이미지의 경우, docker-compose 파일에서 찾을 수 있는 커스텀 이미지를 빌드하려는 경우 'docker-compose build'를 사용하여 이를 수행할 수 있습니다.
...
- 실행 중인 frondend, backend, mongo 컨테이너가 있습니다. 그리고, 그들의 이름도 볼 수 있습니다.

<br>
- frontend 컨테이너의 이름은 docker-complete_frontend_1입니다. 여기에 backend의 이름이 있고, mongo 컨테이너의 이름은 여기에 있죠.

<br>
- 이것은 우리가 도커 컴포즈에 정의한 이름이 아니라는 겁니다. 기술적으로 단지 서비스의 이름일 뿐이며 컨테이너로 번역됩니다.
